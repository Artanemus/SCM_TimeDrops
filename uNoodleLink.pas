...
uses
  System.Classes, Vcl.Controls, BaseGrid, AdvGrid, DBAdvGrid, System.Types; // Assuming TMS Grid unit name
...
type
  TNoodleLink = class
  private
    FSourceGrid: TDBAdvGrid;
    FSourceRow: Integer;
    FDestGrid: TDBAdvGrid;
    FDestRow: Integer;
    FIsSelected: Boolean;
    FSelectedHandle: TLinkEndPointType; // Indicates if source or dest handle is grabbed
    FUserData: TObject;
  public
    constructor Create(ASourceGrid: TDBAdvGrid; ASourceRow: Integer;
                       ADestGrid: TDBAdvGrid; ADestRow: Integer);
    destructor Destroy; override;

    // Function to check if a point is near this link's line or handles
    function HitTest(P: TPoint; PaintBox: TGraphicControl; SagFactor: Single;
        LineTolerance, HandleRadius: Integer; out HitHandle: TLinkEndPointType):
        Boolean;
    function GetEndPointPosition(EndPointType: TLinkEndPointType; PaintBox:
        TGraphicControl): TPoint;

    property SourceGrid: TDBAdvGrid read FSourceGrid write FSourceGrid;
    property SourceRow: Integer read FSourceRow write FSourceRow;
    property DestGrid: TDBAdvGrid read FDestGrid write FDestGrid;
    ...
    function GetGridDotPosition(AGrid: TDBAdvGrid; ARow, ADotColumn: Integer;
      APaintBox: TGraphicControl): TPoint; // Optional user data

implementation

uses
  System.SysUtils, System.Math, Vcl.Graphics, Vcl.Forms;
// Make sure System.Math is in the implementation uses clause
...
function GetGridDotPosition(AGrid: TDBAdvGrid; ARow, ADotColumn: Integer;
  APaintBox: TGraphicControl): TPoint;
var
  CellRect: TRect;
  GridScreenPos, PaintBoxScreenPos: TPoint;
begin
  Result := Point(-1, -1); // Default invalid point
  if (AGrid = nil) or (APaintBox = nil) or (ARow < AGrid.FixedRows) or (ARow >=
    AGrid.RowCount) then
    Exit;
...
constructor TNoodleLink.Create(ASourceGrid: TDBAdvGrid; ASourceRow: Integer;
  ADestGrid: TDBAdvGrid; ADestRow: Integer);
begin
  inherited Create;
...
function TNoodleLink.GetEndPointPosition(EndPointType: TLinkEndPointType;
    PaintBox: TGraphicControl): TPoint;
var
  Grid: TDBAdvGrid;
  Row: Integer;
...
begin
  ...
  const cSourceDotCol = 6; // Example: Dot is in column 1 of the source grid
  const cDestDotCol = 1;   // Example: Dot is in column 0 of the destination grid
...
function TNoodleLink.HitTest(P: TPoint; PaintBox: TGraphicControl; SagFactor:
    Single; LineTolerance, HandleRadius: Integer; out HitHandle:
    TLinkEndPointType): Boolean;
var
